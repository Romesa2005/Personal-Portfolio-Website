...
const Logo = () => {
  ...
  const bgRef = useRef();
  // Ref for the SVG outline path animation
  const outlineLogoRef = useRef();
  // Ref for the solid logo image
  const solidLogoRef = useRef();

  // Run GSAP animations on component mount
  useEffect(() => {
    // Register the DrawSVGPlugin with GSAP
    ...
    gsap
      ...
      .to(bgRef.current, {
        duration: 1,
        opacity: 1, // Fade in the background container
      })
      .from(outlineLogoRef.current, {
        drawSVG: 0, // Animate SVG path drawing
        duration: 20,
      });

    // Fade in the solid logo after 4 seconds
    gsap.fromTo(
